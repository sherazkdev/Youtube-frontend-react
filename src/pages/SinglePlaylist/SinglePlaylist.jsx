    ...
    return (
        <>
            {playlistFetchingLoading == false && data && (
                <section id="watchLaterSection" className="grid grid-cols-[360px_1fr] grid-rows-[minmax(200px,800px)_1fr] gap-[1rem]">
                            {/* playlist panel */}
                            { list === "WL" ? (
                                ...
                                <aside style={gradientStyle} className={`p-4 rounded-lg border border-gray-300 row-span-2`}>
                                    {/* last added video in watch later thumbnail */}
                                    <div className="w-[312px] h-[175px]">
                                        {playlistData?.likeVideos?.length > 0 && (
                                            <img
                                                ref={latestImgRef}
                                                crossOrigin="anonymous"
                                                src={playlistData?.likeVideos[Math.floor(Math.random() * (playlistData?.likeVideos?.length -1))]?.video?.thumbnail}
                                                alt="Latest Video Thumbnail"
                                                className="w-full h-full object-cover rounded-lg"
                                            />
                                    ...
                                    <div className="grid row-span-2 gap-y-[1.5rem]">
                                        {/* playlist name */}
                                        <div>
                                            <h2 className="text-[#ffffff] font-bold text-[28px]">Liked Videos</h2>
                                        </div>
                                        ...
                                        <div className="grid row-span-3 gap-y-3">
                                            {/* owner name + stats */}
                                            <div className="grid row-span-2 gap-y-1">
                                            <h3 className="text-[#ffffff] text-[16px] font-normal">{playlistData?.fullname}</h3>
                                            <div className="flex text-[#ffffffb3] text-[14px] gap-x-1 font-normal">
                                                <p>{playlistData?.likeVideos?.length} videos</p>
                                                <span>•</span>
                                                <p>No views</p>
                                                <span>•</span>
                                                <p>Last updated in {<DateCalculate localDate={playlistData?.updatedAt} />}</p>
                                            </div>
                                            </div>
                            ...
                            { list === "WL" ? (
                                ...
                                <section className="grid w-[97%] grid-rows-[minmax(30px,50px)1fr] row-gap-[1.5rem]" style={{margin:"0px"}}>
                                    ...
                                    {/* video list */}
                                    <div className="h-full w-full flex flex-col gap-y-[1rem]">
                                        {playlistData?.likeVideos?.length > 0 ? playlistData?.likeVideos?.map( (video) => (
                                            <WatchHistoryTypeCard video={video?.video}  key={video?._id}/>
                                        )) : (<>No videos in watch later</>)}
                                    </div>
                                </section>
                            ) : ""}
